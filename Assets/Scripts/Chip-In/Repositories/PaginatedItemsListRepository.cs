    ...
    where TRequestResponseModelInterface : class {
        ...
        public override async Task LoadDataFromServer () {
            try {
                ...
                void LogPageLoadingError (BaseRequestProcessor<object, TRequestResponseDataModel, TRequestResponseModelInterface>.HttpResponse response) {
                    LogUtility.PrintLogError (Tag, $"Failed to load initial repository data from server. Responce Phrase: {response.ResponsePhrase}, Error message: {response.Error}");
                }
        ...
        private Task CreateLoadAndStorePageItemsTask (uint pageNumber) {
            ...
            var httpResponse = CreateAndRegisterLoadPaginatedItemsTask (new PaginatedRequestData ((int) pageNumber,
                itemsPerPage)).ContinueWith (delegate (Task<BaseRequestProcessor<object, TRequestResponseDataModel, TRequestResponseModelInterface>.HttpResponse> task) {
